...
const categoryOptions = computed(() => {
  ...
  return withAllOption(knowledgeHubItems.value.map((item) => item.category))
})
...
const countryOptions = computed(() => {
  ...
  return withAllOption(knowledgeHubItems.value.flatMap((item) => normalizeList(item.country)))
})

const filteredKnowledgeHubItems = computed(() => {
  const items = knowledgeHubItems.value.filter((item) => {
...
const cardGridClass = (index: number) => {
  if (filteredKnowledgeHubItems.value.length === 1) {
  ...
  <section class="bg-white px-5 pb-20 pt-8 font-sans text-black sm:px-8 lg:px-20 lg:pb-28">
    <div class="mx-auto">
      ...
      <div class="grid grid-cols-1 gap-6 lg:grid-cols-12 lg:gap-8">
        <NuxtLink
          v-for="(item, index) in filteredKnowledgeHubItems"
          :key="item.id"
          :to="item.href"
          :class="[
